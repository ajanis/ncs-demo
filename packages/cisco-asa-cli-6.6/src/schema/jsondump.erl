...
emit_mod_info(Mod, Fd) ->
    ...
    Imports = [io_lib:format("\"~s\": \"~s\"", [?a2l(Prefix), ?a2l(ModuleName)]) || {ModuleName , _, Prefix} <- Mod#module.imports],
    io:format(Fd, "        ~s~n      },~n", [string:join(Imports, ",\n        ")]),
...
emit_type_spec(_, #enumeration_type_spec{enums = Enums}, Indent) ->
    ...
    io_lib:format("{~n~s  \"typename\": \"enumeration\",~n~s~n~s}", [Indent, TokenStr, Indent]);
...
join_names(Names, Sep) ->
    string:join(lists:map(fun(E) -> io_lib:format("\"~s\"", [kw2str(E)]) end, Names), Sep).
